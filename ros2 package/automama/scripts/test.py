...
class StereoCameraViewer(Node):
    ...
    def get_pixels_with_disparity(self, disparity_map, row_step=1, col_step=10, min_disparity=10, max_disparity=20):
        """
        Get the pixel positions and their corresponding disparity values from the disparity map
        where the disparity value is within the specified range (min_disparity < disparity < max_disparity),
        while skipping rows and columns based on the given step intervals.
        ...
        - max_disparity: Maximum valid disparity value (exclusive).

        Returns:
        - pixel_positions: A NumPy array (list of lists) of (x, y) pixel coordinates where disparity is within the specified range
        ...
        for row in rows_to_process:
            # Get valid pixel positions where disparity is within the specified range in the selected row
            valid_cols = np.argwhere((disparity_map[row, :] > min_disparity) & (disparity_map[row, :] < max_disparity))
    ...
    def compute_3d_points_from_rays_and_disparities(self,rays, disparities, focal_length=554, base_length=0.6):
        """
    ...
    def show_images(self):
        ...
        point_canvas = scene.SceneCanvas(keys='interactive', size=(800, 600), title="PCD Visualization", show=True)
        point_view = point_canvas.central_widget.add_view()
        point_view.camera = scene.cameras.TurntableCamera()
        ...
        while rclpy.ok():
            ...
            if self.frame_camera1 is not None and self.frame_camera2 is not None:
                ...
                try:
                    ...
                    # print(f'Rays: {rays[:5]}')
                    print(f'disparities: {disparity_values[:500]}')
                    ...
                    scatter.set_data(pcd, face_color='green', size=5)
                    point_canvas.update()
