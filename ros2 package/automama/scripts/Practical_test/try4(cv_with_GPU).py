...
def process_image_gpu(image):
    """
    Converts an image to grayscale, applies Canny edge detection on GPU, and returns the edge map.
    
    Parameters:
    - image: A 640x480 BGR image (NumPy array).
    
    Returns:
    - edge_map: A 640x480 binary image (edges detected).
    """
    # Step 1: Upload image to GPU
    gpu_image = cv2.cuda_GpuMat()
    gpu_image.upload(image)

    # Step 2: Convert the image to grayscale (GPU version)
    gpu_gray_image = cv2.cuda.cvtColor(gpu_image, cv2.COLOR_BGR2GRAY)

    # Step 3: Apply Canny edge detection (GPU version)
    # The threshold values (100, 200) are chosen arbitrarily, adjust based on your needs
    gpu_edge_map = cv2.cuda.Canny(gpu_gray_image, 100, 200)

    # Step 4: Download the edge map back to CPU (NumPy array)
    edge_map = gpu_edge_map.download()
...
def update(event):
    ...
    edges1 = process_image_gpu(left_rgb)
    edges2 = process_image_gpu(right_rgb)
    # Update images
