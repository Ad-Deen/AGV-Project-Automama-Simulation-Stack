...
class StereoCameraViewer(Node):
    ...
    def get_pixels_with_disparity(self,disparity_map,row_step=20,col_step = 1):
        """
        Get the pixel positions in the disparity map where the disparity value is greater than 0,
        ...
        # Initialize a list to collect valid pixel positions
        valid_positions = []
        ...
        for row in rows_to_process:
            ...
            # Convert to (y, x) format and append to the result
            valid_positions.extend([(row, col[0]) for col in cols_to_process])
        ...
        while rclpy.ok():
            ...
            if self.frame_camera1 is not None and self.frame_camera2 is not None:
                ...
                try:
                    ...
                    pixel_positions = self.get_pixels_with_disparity(disparity_map)
                    ...
                    ray_lines.set_data(rays, connect='segments', color='blue', width=1)
                    ray_canvas.update()
